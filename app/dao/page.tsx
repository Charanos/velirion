...
export default function DaoPage() {
  ...
  const handleSubmit = async (values: ProposalFormValues) => {
    try {
      const targets =
        values.target && values.target.length > 0 ? [values.target] : [];
    ...
    <PageShell title="DAO governance" subtitle="Coordinate Velirion proposals">
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <Card className="border-none bg-gradient-to-br from-purple-950/70 via-indigo-950/70 to-zinc-950/70 text-white shadow-sm backdrop-blur-lg">
          <CardHeader>
            <CardTitle className="text-lg font-heading">Create proposal</CardTitle>
        ...
        <Card className="border border-white/10 bg-zinc-900/70 text-white shadow-sm backdrop-blur-lg">
          <CardHeader>
            <CardTitle className="text-lg font-heading">DAO metrics</CardTitle>
            <CardDescription className="text-xs text-white/60">
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <Card className="border border-white/10 bg-zinc-900/70 text-white shadow-sm backdrop-blur-lg">
          <CardHeader>
            <CardTitle className="text-lg font-heading">Proposals</CardTitle>
            <CardDescription className="text-xs text-white/60">
                ...
                {proposals.map((proposal, index) => (
                  ...
                  >
                    ...
                    <div className="mt-3 grid gap-2 sm:grid-cols-3">
                      <Button
                        variant="outline"
                        className="rounded-xl border-white/20 text-white cursor-pointer font-heading"
                    ...
                    <div className="mt-3 grid gap-2 sm:grid-cols-3">
                      <Button
                        className="rounded-xl bg-white/10 cursor-pointer font-heading"
                        onClick={() =>
                          actionHandler(
                            queue,
                            proposal.id,
                      ...
                      <Button
                        className="rounded-xl bg-rose-500/20 text-rose-200 cursor-pointer font-heading"
                        onClick={() =>
                          actionHandler(
                            cancel,
                            proposal.id,
        ...
        <Card className="border-none bg-gradient-to-br from-indigo-950/70 to-purple-950/70 text-white shadow-sm backdrop-blur-lg">
          <CardHeader>
            <CardTitle className="text-lg font-heading">Governance checklist</CardTitle>
            <CardDescription className="text-xs text-white/60">
